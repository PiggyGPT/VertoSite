...
import { 
  User, Settings, ChevronDown, Zap, Lock, ArrowRight, Globe, Server,
  Smartphone, Landmark, ScanLine, CheckCircle, 
  Fingerprint, RefreshCw, FileText 
} from "lucide-react";
...
const PaymentSelection = ({ method, onPay }: { method: 'coinbase' | 'bank', onPay: boolean }) => (
  <div className="flex flex-col h-full p-6">
    ...
    <button className={`mt-auto w-full py-3 text-white rounded-xl font-semibold transition-all duration-200 ${onPay ? 'bg-purple-700 scale-95' : 'bg-purple-500 hover:bg-purple-700'}`}>
      Pay €120.00
    </button>
  </div>
);
